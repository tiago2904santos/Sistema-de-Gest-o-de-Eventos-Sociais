/* V2 — Solicitações de Eventos Sociais (lista) · desktop, estado com dados
...
const est={ordem:'data',desc:true,fila:'',sel:null,painel:false,
...
function per(l){
  ...
  const[di,mi,ai]=l.ini.split('/'),[df,mf,af]=l.fim.split('/');
  if(ai===af&&mi===mf) return `${di}<em>–</em>${df}/${mf}/${af}`;
  if(ai===af) return `${di}/${mi}<em>–</em>${df}/${mf}/${af}`;
  return `${l.ini}<em>–</em>${l.fim}`;
}
function filtradas(){
...
const rotuloOrdem=()=>{const o=ORDENS.find(o=>o.k===est.ordem&&o.desc===est.desc);return o?o.r:`${COLUNAS.find(c=>c.k===est.ordem).r} ${est.desc?'↓':'↑'}`};
...
function render(){
  const rows=ordenadas(filtradas());
  const filtrando=ativos().length>0;
  const total=filtrando?rows.length:(FILAS.find(f=>f.chave===est.fila)?.total??186);

  document.querySelector('#filas').innerHTML=`
    <div class="filas__t"><span class="eyebrow">Filas de trabalho</span></div>
    <div class="filas__lista">
      ${FILAS.map((f,i)=>`${i>0&&f.grupo!==FILAS[i-1].grupo?'<div class="filas__sep"></div>':''}
        <button class="fila${f.destaque?' fila--destaque':''}" data-fila="${f.chave}"${est.fila===f.chave?' aria-current="page"':''}>
          <span class="fila__nome">${f.rotulo}</span><span class="fila__n">${f.total}</span></button>`).join('')}
    </div>`;

  document.querySelector('#prancha').innerHTML=`
  <div class="toolbar">
    ...
    <span class="toolbar__risco" aria-hidden="true"></span>
    ${fc('status','Todos',Object.values(STATUS))}
    ${fc('municipio','Todos',MUNICIPIOS.slice(0,8))}
    ${fc('tipo','Todos',TIPOS)}
    <button class="fc${nAvancados()?' fc--ativo-forte':''}" data-painel aria-expanded="${est.painel}" aria-controls="filtros-avancados">
      ${s('filter')}Filtros${nAvancados()?`<span class="fc__n">${nAvancados()}</span>`:''}${s('chevron-down')}
    </button>
    <div class="toolbar__fim">
      <div class="dd"><button class="ib tip" data-tip="Colunas" data-dd="colunas" aria-haspopup="true" aria-expanded="false" aria-label="Colunas visíveis">${s('columns')}</button>
      ...
      <button class="ib tip" data-tip="Exportar CSV" aria-label="Exportar CSV">${s('download')}</button>
    </div>
  </div>
  <div class="fpanel" id="filtros-avancados"${est.painel?'':' hidden'}>
    ...
    <div class="fpanel__pe">
      <span class="fpanel__dica">Os filtros são aplicados automaticamente ao alterar cada campo.</span>
      <div class="fpanel__acoes">
        ${filtrando?'<button class="btn btn--quieta btn--sm" data-limpar>Limpar filtros</button>':''}
        <button class="btn btn--secundaria btn--sm" data-painel>Recolher</button>
      </div>
    </div>
  </div>
  <div class="meta">
    <div class="meta__esq"><span><b>${total}</b> resultado${total===1?'':'s'}</span>
      ${filtrando?`<div class="chips">${ativos().map(([k,v])=>`<span class="chip"><span>${ROTULOS[k]}:</span>${(k==='inicio'||k==='fim')?brl(v):v}<button data-remover="${k}" aria-label="Remover filtro ${ROTULOS[k]}">${s('x')}</button></span>`).join('')}
        <button class="btn btn--quieta btn--sm" data-limpar>Limpar</button></div>`:''}
    </div>
    <div class="dd"><button class="meta__ordem" data-dd="ordem" aria-haspopup="true" aria-expanded="false">${rotuloOrdem()}${s('chevron-down')}</button>
      <div class="dd__c" hidden data-dd-c="ordem"><div class="dd__t">Ordenar por</div>
        ${ORDENS.map(o=>{const a=o.k===est.ordem&&o.desc===est.desc;return `<button class="dd__i${a?' dd__i--sel':''}" data-ordem="${o.k}:${o.desc}">${a?s('check'):'<span class="dd__vaga"></span>'}${o.r}</button>`}).join('')}</div></div>
  </div>
  ${rows.length?`<div class="dt-w"><table class="dt">
    <thead><tr>
      ${COLUNAS.filter(c=>est.cols[c.k]).map(c=>{const a=est.ordem===c.k;
        return `<th class="${c.cls}"${a?` aria-sort="${est.desc?'descending':'ascending'}"`:''}><button class="dt__ord" data-ordenar="${c.k}">${c.r}${s(a?(est.desc?'arrow-down':'arrow-up'):'ordenar')}</button></th>`}).join('')}
      ...
      ${rows.map(l=>{const a=acao(l.st),sel=est.sel===l.id;
        ...
        return `<tr data-id="${l.id}"${sel?' aria-selected="true"':''} tabindex="0">
          ...
          <td class="c-acoes"><div class="acoes">
            ${a?`<button class="btn btn--secundaria btn--sm">${s(a.i)}${a.r}</button>`:''}
            <div class="dd"><button class="ib ib--sm" data-dd="k${l.id}" aria-haspopup="true" aria-expanded="false" aria-label="Mais ações da solicitação #${l.id}">${s('kebab')}</button>
  ...
  :`<div class="semres">
      ...
      <button class="btn btn--secundaria btn--sm" data-limpar>Limpar filtros</button>
    </div>`}`;
}
...
document.addEventListener('click',e=>{
  ...
  fechar();
  const tr=q('tbody tr');
  if(tr&&!q('.btn')&&!q('.ib')){est.sel=est.sel===+tr.dataset.id?null:+tr.dataset.id;render()}
});
