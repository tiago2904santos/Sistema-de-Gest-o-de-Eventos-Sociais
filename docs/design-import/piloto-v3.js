/* V3 — Solicitações de Eventos Sociais (lista) · desktop, estado com dados
...
function render(){
  ...
  <div class="cabeca">
    ...
    <div class="cabeca__dir">
      <div class="figuras">
        <div class="figura"><span class="figura__n">186</span><span class="figura__r">Total</span></div>
        <div class="figura figura--aten"><span class="figura__n">14</span><span class="figura__r">Aguardando despacho</span></div>
        <div class="figura"><span class="figura__n">5</span><span class="figura__r">Devolvidas para ajuste</span></div>
      </div>
      <div class="cabeca__acao"><button class="btn btn--primaria">${s('plus')}Nova solicitação</button></div>
    </div>
  </div>`;
  ...
  <div class="fpanel" id="filtros-avancados"${est.painel?'':' hidden'}>
    ...
    <div class="fpanel__pe">
      ...
      <div class="fpanel__acoes">
        ${filtrando?'<button class="btn--quieta btn--sm" data-limpar>Limpar filtros</button>':''}
        <button class="btn btn--secundaria" data-painel>Recolher</button>
      </div>
    </div>
  </div>`;
  ...
  <div class="meta">
    <div class="meta__esq"><span><b>${total}</b> resultado${total===1?'':'s'}</span>
      ${filtrando?`<div class="chips">${ativos().map(([k,v])=>`<span class="chip"><span>${ROTULOS[k]}:</span>${(k==='inicio'||k==='fim')?brl(v):v}<button data-remover="${k}" aria-label="Remover filtro ${ROTULOS[k]}">${s('x')}</button></span>`).join('')}
        <button class="btn--quieta btn--sm" data-limpar>Limpar</button></div>`:''}
    </div>
    <div class="dd"><button class="meta__ordem" data-dd="ordem" aria-haspopup="true" aria-expanded="false">${rotuloOrdem()}${s('chevron-down')}</button>
      <div class="dd__c" hidden data-dd-c="ordem"><div class="dd__t">Ordenar por</div>
        ${ORDENS.map(o=>{const at=o.k===est.ordem&&o.desc===est.desc;return `<button class="dd__i${at?' dd__i--sel':''}" data-ordem="${o.k}:${o.desc}">${at?s('check'):'<span class="dd__vaga"></span>'}${o.r}</button>`}).join('')}</div></div>
  </div>`;

  const tabela=rows.length?`<div class="dt-w"><table class="dt">
    <thead><tr>
  ...
  document.querySelector('#folha').innerHTML=bandaCabeca+bandaFilas+bandaBarra+bandaMeta+tabela;
}
